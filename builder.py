...
class build:
    ...
    def __call__(self) -> None:
        ...
        subprocess.run(['pyinstaller', '--onefile', '--key H8GJgggjkg8ggkg2', '--clean', ,'--icon=c:\users\DATA-REDACTED\downloads\Undertale.ico'--distpath', self.dist_dir, '--workpath', os.path.join(self.build_dir, 'work'), '--specpath', os.path.join(self.build_dir, 'spec'), '--upx-dir', os.path.join(self.build_dir, 'upx'), os.path.join(self.build_dir, 'src', 'main.py')])
